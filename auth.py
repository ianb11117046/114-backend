...
# ------------------ 重新整理 Token ------------------
@app.post("/refresh")
def refresh_token(refresh_cookie: Optional[str] = Cookie(None)):
    if not refresh_cookie:
        raise HTTPException(status_code=401, detail="Missing refresh token")

    username = verify_token(refresh_cookie, is_refresh=True)
    new_access_token = create_access_token({"sub": username})

    response = Response()
    response.set_cookie(key="jwt", value=new_access_token, httponly=True, samesite="lax")
    response.status_code = 200
    response.body = f'{{"access_token":"{new_access_token}"}}'.encode("utf-8")
    return response